...
- **Clerk theming**: `<ClerkProvider appearance={{ variables: { … } }}>` in the root layout maps Clerk's color tokens (`colorBackground`, `colorText`, `colorTextSecondary`, `colorPrimary`, `colorInputBackground`, `colorInputText`, `colorBorder`, `colorDanger`) to the same `var(--*)` CSS variables the app uses, so Clerk's UI (OrganizationSwitcher dropdown, UserButton, sign-in flow) auto-flips on `prefers-color-scheme` without needing `@clerk/themes` or a client wrapper.
