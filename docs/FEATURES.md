...
- **Resolvers** (`src/lib/listings/resolve-neighborhood.ts`): two async shims — `resolveNeighborhood` and `resolveDistrict` — that fall back through (1) parser output, (2) the existing column value (when called from refresh), (3) Nominatim reverse-geocode of the listing's lat/lng. Nominatim's `address` response maps `city_district` / `borough` / `district` → district (Chicago community-area level, e.g. "Near West Side") and `neighbourhood` / `suburb` / `quarter` → neighborhood. Caching is implicit: once a value sticks in the column, subsequent refreshes see it via `current` and skip Nominatim. Used for both `createListingFromUrl` and `refreshListing`.
