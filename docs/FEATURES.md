...
- **Translation**: `<GoogleTranslate>` (`src/components/google-translate.tsx`) mounts a Google Translate widget in the header for all signed-in users. It uses two `next/script` tags with `lazyOnload` strategy — an inline script registers `window.googleTranslateElementInit`, the external `translate.google.com/translate_a/element.js` calls it on load. Source language is `en`; included target is `pt` (Portuguese — Google's translation engine produces Brazilian Portuguese by default). When the user picks Portuguese the widget translates the page in place and persists the choice via the `googtrans` cookie. Tradeoff: the widget injects a "Translated by Google" banner near the top of the page when active and may interfere with React's reconciliation in some edge cases — if that becomes annoying we'd switch to `next-intl` with an explicit catalog.
